...
def handle_store(event):
    ...
    biobanca = args.port is 104

    # Check if study comes from LAN
    LAN = ipaddress.ip_address(callingIP) in ipaddress.ip_network('192.168.179.0/24')

    # *************************************************************************

    # Da RIMUOVERE!!!  Serve solo per testing da VPN che non sia biobanca.
    biobanca = False    
    APP_LOGGER.info('biobanca? {}'.format(biobanca))

    # Use:
    # QCS_TEST  -> testing DEID + qTC
    # QCS       -> testing DEID + qTC + send2Pacs 228
    # XNAT_TEST -> testing DEID + Deid XNAT + send2Xnat

    StationAETitle_QCS  = "QCS_TEST"
    ...
    if biobanca:
        ...
        if LAN:
            ...
            # aggiungere lo studio del DB-Deid
            ...
            '''
            SOSTITUIRE SE POSSIBILE LA DEIDENTIFICAZIONE CON LA CHIAMATA AL WEBSERVICE DI DEIDENTIFICAZIONE
            '''
        ...
        try:
            ...
            return status_ds
        APP_LOGGER.info('Insert tag StationAETitle = {}'.format(StationAETitle_QCS))
        ds.add_new(0x00080055, 'AE', StationAETitle_QCS)
        ...
        if resp['success'] is True:
            ...
            if resp['data'] == []: # Nessun StudyInstaceUID Trovato!
                ...
                if not biobanca:
                    ...
                    qtcBuild = qtcExamBuild (pN,deidExam,qtcClinicOid,ds,APP_LOGGER)
